...
from FedNova import *
from sam import SAM
from models import VGG, ImageNet_MobileNetV2,ImageNet_ResNet
import random
import logging
import datetime
import torchsummary
...
def main_test(args):
    ...
    save_path_tmp = os.path.join(args.save_path, 'tmp_{}_{}_{}_T[{}]_C[{}]_iid[{}]_E[{}]_B[{}]_GPU[{}]_Pruned[{}]_{}'.
                             format(args.dataset, args.model,args.norm, args.epochs, args.frac, args.iid,
                                    args.local_ep, args.local_bs,args.gpu,args.pruning,now))
    if not os.path.exists(save_path_tmp):
        os.makedirs(save_path_tmp)
    SAVE_PATH = os.path.join(args.save_path, '{}_{}_{}_T[{}]_C[{}]_iid[{}]_E[{}]_B[{}]_GPU[{}]_Pruned[{}]'.
                             format(args.dataset, args.model,args.norm, args.epochs, args.frac, args.iid,
                                    args.local_ep, args.local_bs,args.gpu,args.pruning))
...
#    torch.cuda.set_device(0)
    ...
    if args.model == 'cnn':
        ...
        #tiny imagenet
        global_model = tiny_mobilenet_v2(pretrained= False, pruning_ratio = 1)
        #m model 형태
        #m = torch.load("MMV2_LBYL_61.73.pt")
        #global_model.load_state_dict(m.state_dict())
    elif args.model == 'mobilenet_v2':
        pruning_ratio = args.pruning
        global_model = ImageNet_MobileNetV2.mobilenet_v2(pretrained=False, pruning_ratio = 1 - pruning_ratio,num_classes=10)
        #m state_dict 형태
        #m = torch.load("Lp{}.pt".format(int(args.pruning*100)))
        #global_model.load_state_dict(m)
    elif args.model == 'resnet34':
        cfg = [64, 128, 256, 512]
        for i in range(len(cfg)):
            cfg[i] = int(cfg[i] * (1 - args.pruning))
        temp_cfg = cfg
        global_model = ImageNet_ResNet.resnet34(pretrained= False, cfg=cfg)
        #m = torch.load("Prune_{}_ResNet34prune.pt".format(int(args.pruning*100)))
        m = torch.load("ResNet34OURS_{}.pt".format(args.pruning))
        global_model.load_state_dict(m)
    else:
    ...
    if args.dataset == 'fedcifar100':
        ...
        loggertxt.info(torchsummary.summary(copy.deepcopy(global_model),(3,224,224),device='cpu'))
    else:
        loggertxt.info(torchsummary.summary(copy.deepcopy(global_model),(3,32,32),device='cpu'))
    ...
    for epoch in tqdm(range(resume_iter, args.epochs)):
        ...
        for it, idx in enumerate(idxs_users):
            ...
            local_model = LocalUpdate(args=args, train_loader=client_loader_dict[idx], device=device)
            w, loss = local_model.update_weights(model=copy.deepcopy(global_model), global_round=epoch, idx_user=idx)
        
            '''if (epoch+1) %50 == 0: 
                with torch.no_grad():
                    total_acc,personal_acc = local_model.personal_infernece(client_class_set[idx],test_dataset)
                loggertxt.info("client:{}, personal acc: {:.2f}%".format(idx, 100 * personal_acc))
                personal_acc_total += personal_acc'''
            # qffl
        ...
        #if (epoch+1) %50 == 0: 
        #     loggertxt.info("personal total acc: {:.2f}%".format(100 * personal_acc_total / len(idxs_users)))
       ...
       # global_weights = average_weights_class_normalize(local_weights,client_loader_dict,idxs_users,torch.tensor(client_indicate_class).to(device))
        ...
        #global_weights = server_fine_tuning(global_weights,global_model,epoch,syn_round_dataset,device,args)
        # 원래 위치
       ...
       # q_loss = qffl_infernece(args,global_model,real_dataset,device)
        ...
        '''if (epoch+1) % 60 == 0:
            for idx, param_group in enumerate(opt.param_groups):
                opt.param_groups[idx]['lr'] =  opt.param_groups[idx]['lr'] * 0.1
                print(opt.param_groups[idx]['lr'])
        if (epoch+1) % 90 == 0:
            for idx, param_group in enumerate(opt.param_groups):
                opt.param_groups[idx]['lr'] =  opt.param_groups[idx]['lr'] * 0.1
                print(opt.param_groups[idx]['lr'])'''
       ...
       # global_model.load_state_dict(tmp_model[1:])
        ...
        if args.client_decay:
            args.lr = exp_decay_schedule_builder(args.lr, 3, 0.1, True, epoch+1)
        ...
        if (epoch+1) % 1 ==0:
            ...
            #if args.kernel_sizes == 'centering':
    ...
    total_list = [val_acc_list,round_indicate_class,accumlate_indicate_class]
